<template>
  <div>
    <!--表格栏-->
    <el-table :data="data.rows" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" :header-cell-style="headerCellStyle"
      ...
      <el-table-column :label="$t('action.operation')" min-width="180" :width="buttonWidth" fixed="right" v-if="showOperation" header-align="center" align="center">
        <template slot-scope="scope">
          <kt-button icon="fa fa-edit" :label="$t('action.resetPwd')" :perms="permsResetPwd" v-if="resetPwdShow" :size="size" @click="handleResetPwd(scope.$index, scope.row)" />
    ...
    <div class="toolbar" style="padding:10px;">
      ...
      <el-pagination layout="total, prev, pager, next, jumper" @current-change="refreshPageRequest"
        :current-page="pageRequest.page" :page-size="pageRequest.limit" :total="data.totalSize" style="float:right;">
      </el-pagination>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleResetPwd: function (index, row) {
      this.$emit('handleResetPwd', {index:index, row:row})
    },
    // 删除
